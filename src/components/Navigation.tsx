...
const Navigation = () => {
  ...
  const navItems = [
    ...
    { id: 'skills', label: 'Skills', isExternal: false },
    { id: 'experience', label: 'Experience', isExternal: false },
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = navItems.filter(item => !item.isExternal).map(item => item.id);
  ...
  const handleNavClick = (item: typeof navItems[0]) => {
    if (item.isExternal) {
      ...
      // Scroll to section for internal navigation
